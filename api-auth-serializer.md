...
API 的安全配置定义在 [security.yaml](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/config/packages/security.yaml#L20-L31) 中，核心配置：
...
- **访问控制**：`^/api` 路径需要 `IS_AUTHENTICATED_REMEMBERED` 角色（见第 102 行）

### 2.2 请求匹配器 ApiRequestMatcher

[ApiRequestMatcher.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/API/Authentication/ApiRequestMatcher.php#L15-L49) 决定哪些请求由 `api` 防火墙处理：
...
新的推荐认证方式，由 [AccessTokenHandler.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/API/Authentication/AccessTokenHandler.php#L17-L45) 实现 `AccessTokenHandlerInterface`：
...
AccessToken 实体定义在 [AccessToken.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/Entity/AccessToken.php#L23-L107)，包含 `token`、`name`、`lastUsage`、`expiresAt` 等字段。`isValid()` 方法检查过期时间。

### 2.4 废弃的 X-AUTH-USER 认证（TokenAuthenticator）

旧的双 Header 认证方式，由 [TokenAuthenticator.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/API/Authentication/TokenAuthenticator.php#L34-L156) 实现，已标记 `@deprecated since 2.54`：
...
迁移机制由 [ApiTokenMigratingListener.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/API/Authentication/ApiTokenMigratingListener.php#L20-L62) 监听 `LoginSuccessEvent`，当 API token 需要重新哈希时自动升级。

### 2.5 认证成功处理器（AccessTokenSuccessHandler）

[AccessTokenSuccessHandler.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/API/Authentication/AccessTokenSuccessHandler.php#L17-L24) 在 Bearer Token 认证成功后设置一个令牌属性：
...
[ApiVoter.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/Voter/ApiVoter.php#L24-L78) 是 API 访问权限的核心守卫，控制器上的 `#[IsGranted('API')]` 最终由它处理：
...
- [UserEnvironmentSubscriber.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/EventSubscriber/UserEnvironmentSubscriber.php#L70-L77)：注入用户环境
- [PasswordResetSubscriber.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/EventSubscriber/PasswordResetSubscriber.php#L26-L29)：密码重置重定向
- [ThemeOptionsSubscriber.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/EventSubscriber/ThemeOptionsSubscriber.php)：主题选项

### 3.2 UserEnvironmentSubscriber：环境上下文注入

[UserEnvironmentSubscriber.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/EventSubscriber/UserEnvironmentSubscriber.php#L60-L84) 监听 `KernelEvents::REQUEST`（优先级 -10），在请求早期设置用户环境：
...
这是一个非常重要的点：**API 请求也会经过这个订阅者**，所以序列化时的日期格式、时区等都已切换到当前用户的设置。

### 3.3 BaseApiController::getUser()：控制器中的用户获取

[BaseApiController.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/API/BaseApiController.php#L28-L36) 提供了类型安全的用户获取方法：
...
[BaseApiController.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/API/BaseApiController.php#L68-L112) 的 `prepareQuery()` 方法将当前用户注入查询对象：
...
在 [UserController.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/API/UserController.php#L109-L114) 中，获取单个用户时会派发 `PrepareUserEvent`：
...
- [jms_serializer.yaml](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/config/packages/jms_serializer.yaml)：JMS Serializer 配置
- [fos_rest.yaml](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/config/packages/fos_rest.yaml)：FOSRestBundle 配置
...
[ViewHandler.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/API/ViewHandler.php#L18-L78) 装饰了 FOSRestBundle 的基础视图处理器，增加了分页支持：
...
以 [UserController.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/API/UserController.php#L41-L44) 为例：
...
以 [Timesheet 实体](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/Entity/Timesheet.php#L46-L49) 为例，通过虚拟属性实现展开/折叠：
...
以 [User 实体](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/Entity/User.php#L42-L43) 为例：
...
[ValidationFailedExceptionErrorHandler.php](file:///d:/fz/0601-2/solo-dogfeeding/code/18-kimai/src/API/Serializer/ValidationFailedExceptionErrorHandler.php#L24-L99) 是一个 JMS Serializer 订阅处理器，负责将验证失败异常序列化为友好的 JSON 格式。
...
| 用户上下文 | `UserEnvironmentSubscriber` | 注入时区/语言/权限 | 从 TokenStorage 读用户 → 设置全局环境 |
...
5. **最小化数据库写入**：AccessToken 的 `lastUsage` 每分钟才更新一次，避免每次 API 请求都写数据库。
