...
| 类 | 职责 |
|---|---|
| [LdapAuthenticator](src/Ldap/LdapAuthenticator.php) | 认证入口，装饰 Symfony 原生 `form_login` 认证器，在 Passport 上挂载 `LdapBadge` |
| [LdapCredentialsSubscriber](src/Ldap/LdapCredentialsSubscriber.php) | 监听 `CheckPassportEvent`，检测 `LdapBadge` 后执行 LDAP bind + 属性同步 |
| [LdapManager](src/Ldap/LdapManager.php) | 核心业务：LDAP 查询、用户水合（hydrate）、属性映射、角色同步 |
| [LdapUserProvider](src/Ldap/LdapUserProvider.php) | Symfony `UserProviderInterface` 实现；`loadUserByIdentifier` 查 LDAP 水合新用户，`refreshUser` 仅在 DB 查不到时兜底 |
| [LdapDriver](src/Ldap/LdapDriver.php) | 底层 LDAP 操作封装（search / bind），委托 Laminas\Ldap |
| [LdapConfiguration](src/Configuration/LdapConfiguration.php) | 从 `SystemConfiguration` 读取 `ldap.*` 配置节 |
| [KimaiUserProvider](src/Security/KimaiUserProvider.php) | Chain Provider，串联内部数据库 provider 与 LDAP provider |
| [FormLoginLdapFactory](src/Ldap/FormLoginLdapFactory.php) | 安全工厂，注册 `LdapAuthenticator` 和 `LdapCredentialsSubscriber` 到防火墙 |
| [LastLoginSubscriber](src/EventSubscriber/LastLoginSubscriber.php) | 监听 `LoginSuccessEvent`，设置 lastLogin 时间并持久化 User 到数据库 |
...
[file: security.yaml](config/packages/security.yaml)
...
[KimaiUserProvider](src/Security/KimaiUserProvider.php#L35-L54)
...
[LdapCredentialsSubscriber::onCheckPassport()](src/Ldap/LdapCredentialsSubscriber.php#L29-L81)
...
注意 `LdapCredentialsSubscriber::onCheckPassport()` [L64](src/Ldap/LdapCredentialsSubscriber.php#L64-L64) 的调用：
...
`accountFilterFormat` 在 [AppExtension::load() L64-L69](src/DependencyInjection/AppExtension.php#L64-L69) 中自动推导生成：
...
3. LoginSuccessEvent 触发
   → LastLoginSubscriber::onFormLogin() [L42-L49](src/EventSubscriber/LastLoginSubscriber.php#L42-L49)
...
**与 SAML 的对比**：[SamlProvider::findUser()](src/Saml/SamlProvider.php#L33-L61) 在 hydrate 后立即显式调用 `$this->userService->saveUser($user)`，而 LDAP 的登录流程采用「修改内存对象 + LoginSuccessEvent 统一持久化」的解耦模式，refreshUser 阶段则通常不会触发 LDAP 同步。

### 3.4 阶段四：会话刷新

会话刷新由 [KimaiUserProvider::refreshUser()](src/Security/KimaiUserProvider.php#L66-L69) 发起，委托给内部 `ChainUserProvider`，按 `kimai_internal` → `kimai_ldap` 顺序尝试：

```
KimaiUserProvider::refreshUser($user)
  → ChainUserProvider 遍历 providers
    ├─ kimai_internal（EntityUserProvider）
    │   → UserRepository::refreshUser($user) [L171-L177](src/Repository/UserRepository.php#L171-L177)
    ...
    └─ kimai_ldap（LdapUserProvider）
        ...
        → [LdapUserProvider::refreshUser()](src/Ldap/LdapUserProvider.php#L53-L69)
...
[LdapManager::findUserByUsername()](src/Ldap/LdapManager.php#L33-L53)
...
[LdapManager::updateUser()](src/Ldap/LdapManager.php#L82-L123)
...
[LdapManager::hydrateUser()](src/Ldap/LdapManager.php#L147-L175)
...
[LdapManager::hydrateUserWithAttributesMap()](src/Ldap/LdapManager.php#L225-L270)
...
配置格式（[Configuration.php getLdapNode()](src/DependencyInjection/Configuration.php#L782-L789)）：
...
[Configuration.php getLdapNode() role 节](src/DependencyInjection/Configuration.php#L793-L811)
...
[LdapManager::updateUser() 角色部分](src/Ldap/LdapManager.php#L104-L123)
...
[LdapManager::hydrateRoles()](src/Ldap/LdapManager.php#L181-L213)
...
| 场景 | 处理方式 | 代码位置 |
|---|---|---|
| LDAP 属性在条目中不存在 | **静默跳过**，字段保持原值不变 | [hydrateUserWithAttributesMap L230](src/Ldap/LdapManager.php#L230-L232) |
| usernameAttribute 对应属性缺失 | **抛致命异常** `LdapDriverException('Missing username in LDAP hydration')` | [hydrateUserWithAttributesMap L267-L269](src/Ldap/LdapManager.php#L267-L269) |
| email 为 null（未映射或 LDAP 无值） | **兜底为 username** | [hydrateUser L163-L166](src/Ldap/LdapManager.php#L163-L166) |
| email 为数组（LDAP 多值） | **取首个元素** | [hydrateUserWithAttributesMap L252-L255](src/Ldap/LdapManager.php#L252-L255) |
| user_method 不存在于 User 类 | **抛异常** `'Unknown mapping method: ...'` | [hydrateUserWithAttributesMap L260-L262](src/Ldap/LdapManager.php#L260-L262) |
| role.baseDn 为 null | **跳过整个角色同步** | [updateUser L105-L107](src/Ldap/LdapManager.php#L105-L107) |
| role.usernameAttribute 属性不在条目中 | **回退到 'dn'** | [updateUser L110-L112](src/Ldap/LdapManager.php#L110-L112) |
| 角色组名不在 groups 映射 | **自动生成** `ROLE_<SLUGIFIED_NAME>` | [hydrateRoles L202-L204](src/Ldap/LdapManager.php#L202-L204) |
| 生成的角色不在 allowedRoles | **静默忽略** | [hydrateRoles L206-L208](src/Ldap/LdapManager.php#L206-L208) |
| LDAP bind 失败 + 用户为内部用户 | **降级到本地密码校验** | [LdapCredentialsSubscriber L67-L69](src/Ldap/LdapCredentialsSubscriber.php#L67-L69) |
| LDAP bind 失败 + 用户为 LDAP 用户 | **抛 BadCredentialsException** | [LdapCredentialsSubscriber L70](src/Ldap/LdapCredentialsSubscriber.php#L70) |
| 用户搜索返回多条结果 | **抛 LdapDriverException** | [findUserByUsername L43-L44](src/Ldap/LdapManager.php#L43-L44) |
| DN 查找失败 | **抛 LdapDriverException('Failed fetching user DN')** | [updateUser L86-L88](src/Ldap/LdapManager.php#L86-L88) |
| 属性查询返回 0 条结果 | **直接 return，不修改用户** | [updateUser L98-L99](src/Ldap/LdapManager.php#L98-L99) |
| 新用户登录时 id=null（未持久化） | **后续 LoginSuccessEvent 中 saveUser() 写入 DB** | [LastLoginSubscriber L48](src/EventSubscriber/LastLoginSubscriber.php#L48-L48) |
...
| bind 时传入登录名而非 DN（bindRequiresDn=true） | **Laminas 底层用 accountFilterFormat 查 DN 后再绑定**，代码层面传入的是登录名 | [LdapCredentialsSubscriber L64](src/Ldap/LdapCredentialsSubscriber.php#L64-L64) |
...
[LdapCredentialsSubscriber](src/Ldap/LdapCredentialsSubscriber.php#L64-L81) 中存在一个隐含行为：
...
[AppExtension](src/DependencyInjection/AppExtension.php#L60-L70) 在编译容器时做了两步自动补全：
